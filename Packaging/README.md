...
For v4.4, upload the immutable build 422 package as `Nick-4.4.pkg` to:

`https://3nsofts.com/nick/releases/Nick-4.4.pkg`

Then replace the hosted `https://3nsofts.com/nick/appcast.xml` with
`Packaging/Release/v4.4.0/appcast.xml`. The package must not be modified after
...
```sh
PKG_PATH=/Users/Shared/Nick-4.4-build-422.pkg \
OUTPUT_PATH=/Users/Shared/Nick-4.4-build-422.dmg \
...
`Packaging/Release/v4.4.0/SHA256SUMS.txt`. Recreate that file whenever either
