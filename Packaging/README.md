...
For v4.5, upload the immutable build 426 package as `Nick-4.5.pkg` to:

`https://3nsofts.com/nick/releases/Nick-4.5.pkg`

Then replace the hosted `https://3nsofts.com/nick/appcast.xml` with
`Packaging/Release/v4.5.0/appcast.xml`. The package must not be modified after
...
```sh
PKG_PATH=/Users/Shared/Nick-4.5-build-426.pkg \
OUTPUT_PATH=/Users/Shared/Nick-4.5-build-426.dmg \
...
`Packaging/Release/v4.5.0/SHA256SUMS.txt`. Recreate that file whenever either
